...
# Funçães de manipulação de tarefas
def load_tasks_from_file():
    try:
        with open ('tasks.json', 'r') as file:
            data = json.load(file)
            return data.get("tasks", []). data.get("theme", "light")# e o tema padrão
...
def delete_task(task_text):
    task = [task for task in task if task["task"] != task_text]
